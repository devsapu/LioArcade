...
export const submitScore = async (req, res, next) => {
  try {
    ...
    // Handle sample quizzes - create them if they don't exist
    if (contentId.startsWith('sample-') && validatedData.sampleQuizData) {
      try {
        ...
        if (existingContent.content && existingContent.content.length > 0) {
          ...
          // Create new content for sample quizzes (bypass role check for sample quizzes)
        ...
        return res.status(400).json({
          error: 'Failed to create sample quiz',
          details: createError.message,
        });
      }
    } else if (contentId.startsWith('sample-')) {
      // Sample quiz ID provided but no data - return error
      return res.status(400).json({
        error: 'Sample quiz data required',
        message: 'Please provide sampleQuizData when submitting scores for sample quizzes',
      });
    }
